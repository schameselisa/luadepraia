/*
  # Take order ownership from the session, never from the request

  1. Problem
     `place_order(..., p_customer_id uuid)` did
       v_customer_id := COALESCE(p_customer_id, auth.uid());
     so the caller chose which account the order was filed under. Anyone could
     pass a victim's user id and have a forged order appear in that victim's
     "Meus pedidos" list.

  2. Changes
     - Recreate the function so customer_id is ALWAYS auth.uid(). The
       p_customer_id parameter is kept in the signature (so the existing client
       call keeps working unchanged) but is ignored entirely.

  3. Notes
     - Guest checkout still works: auth.uid() is NULL for an anonymous caller,
       which is exactly what the previous guest path stored.
*/
